				...
				tool_calls = {
					{
						...
						["function"] = {
							...
							arguments = vim.json.decode(
								tool_history.tool_use.input_json
							),
						},
					},
				},
			})
			local result_content = tool_history.tool_result.content
				or ""
			table.insert(messages, {
				role = "tool",
				tool_call_id = tool_history.tool_result.tool_use_id,
				content = tool_history.tool_result.is_error
						and "Error: " .. result_content
					or result_content,
			})
		end
	end
	...
	if json_data then
		...
		if json_data.done then
			handler_opts.on_stop({
				reason = json_data.done_reason or "stop",
			})
			return
		end
	end
					...
					openai_fim_compatible = {
						...
						template = {
							...
							suffix = function(_, suffix)
								...
								local ok, cacher = pcall(
									require,
									"vectorcode.cacher"
								)
								if ok then
									local cache =
										cacher.query_from_cache()
									for _, file in ipairs(cache) do
										context = context
											.. "// reference file that is in the same project is as follows: "
											.. file.path
											.. "\n"
											.. file.document
											.. "\n\n"
